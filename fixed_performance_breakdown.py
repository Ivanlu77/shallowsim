...
for gpu_name in tg260_gpus:
    if gpu_name in gpu_dict:
        ...
        components[gpu_name] = {
            'dense_mla': dense_mla,    # 已经是毫秒
            'dense_mlp': dense_mlp,    # 已经是毫秒  
            'tp_mla': tp_mla,          # 已经是毫秒
            'shared': shared,          # 已经是毫秒
            'combine': combine,        # 已经是毫秒
            'routed': routed,          # 已经是毫秒
            'dispatch': dispatch       # 已经是毫秒
        }
...
for comp_name, comp_label in zip(component_names, component_labels):
    ...
    for gpu_name in tg260_gpus:
        if gpu_name in components:
            time_ms = components[gpu_name][comp_name]  # 直接使用，已经是毫秒
            row += f"{time_ms:10.3f}"
        else:
            row += f"{'N/A':10}"
    print(row)
...
for gpu_name in tg260_gpus:
    if gpu_name in components:
        ...
        sum_time_ms = comp_time
        if overlap1 > 0:
            sum_time_ms += overlap1 * n_sparse_layers
        if overlap2 > 0:
            sum_time_ms += overlap2 * n_sparse_layers
        
        # 转换为秒
        sum_time_s = sum_time_ms / 1000
        
        # 计算吞吐量 (tokens/sec/device)
        throughput = seq_len / sum_time_s / (tp * dp)
        
        components[gpu_name]['total_time_ms'] = sum_time_ms
        components[gpu_name]['total_time_s'] = sum_time_s
        components[gpu_name]['throughput'] = throughput

# 显示总时间和吞吐量
print(f"{'总时间(ms)':15}", end="")
for gpu_name in tg260_gpus:
    if gpu_name in components:
        print(f"{components[gpu_name]['total_time_ms']:10.1f}", end="")
    else:
        print(f"{'N/A':10}", end="")
print()
...
for gpu_name in tg260_gpus:
    if gpu_name in components:
        print(f"{components[gpu_name]['total_time_s']:10.3f}", end="")
    else:
        print(f"{'N/A':10}", end="")
print()
...
for gpu_name in tg260_gpus:
    if gpu_name in components:
        ...
        print(f"\n{gpu_name}:")
        print(f"  通信时间 (Combine + Dispatch): {comp['combine'] + comp['dispatch']:.3f}ms")
        print(f"  计算时间 (其他组件): {comp['dense_mla'] + comp['dense_mlp'] + comp['tp_mla'] + comp['shared'] + comp['routed']:.3f}ms")
        print(f"  通信时间占比: {((comp['combine'] + comp['dispatch']) / comp['total_time_ms'])*100:.1f}%")
        print(f"  总时间: {comp['total_time_s']:.3f}s")
        print(f"  吞吐量: {comp['throughput']:.1f} tokens/sec/device") 